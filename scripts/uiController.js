...
export class UIController {
  ...
  applyTheme() {
    if (this.state.theme === 'light') {
      ...
      if (this.els.sunIcon) this.els.sunIcon.classList.remove('hidden');
      if (this.els.moonIcon) this.els.moonIcon.classList.add('hidden');
    } else {
      document.body.classList.remove('light-theme');
      if (this.els.sunIcon) this.els.sunIcon.classList.add('hidden');
      if (this.els.moonIcon) this.els.moonIcon.classList.remove('hidden');
    }
  }
  ...
  async loadSurahsList() {
    ...
    chapters.forEach(ch => {
      ...
      if (ch.id === this.state.surah) opt.selected = true;
  ...
  adjustPreviewContainerRatio() {
    ...
    if (this.state.aspectRatio === '1:1') this.els.mockupContainer.classList.add('aspect-square');
    else if (this.state.aspectRatio === '9:16') this.els.mockupContainer.classList.add('aspect-[9/16]');
    else if (this.state.aspectRatio === '16:9') this.els.mockupContainer.classList.add('aspect-[16/9]', 'max-w-[500px]');
  }
  ...
  async triggerFetch() {
    ...
    try {
      ...
      const verses = await quranService.fetchVerses(
        this.state.surah,
  ...
  updateAudioBtnUI() {
    ...
    if (this.state.isPlayingAudio) {
      this.els.audioPlayBtn.innerHTML = `<svg class="h-3 w-3 fill-current" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>`;
      this.els.audioPlayBtn.classList.add('bg-gold-500', 'text-slate-950');
    } else {
      this.els.audioPlayBtn.innerHTML = `<svg class="h-3 w-3 fill-current" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>`;
  ...
  updateMockupPreview() {
    ...
    if (arabicTextEl) {
      ...
      arabicTextEl.style.fontFamily = fontMap[this.state.arabicFont] || fontMap['Amiri'];
      arabicTextEl.innerHTML = `<div style="font-size: ${layout.arabic.fontSize}px; word-spacing: ${this.state.fontGap}px; line-height: ${layout.arabic.fontSize * 1.5 + this.state.lineGap}px;">${displayAr}</div>`;
    }
